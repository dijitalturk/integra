...
var Html = function(options) {
  ...
  this.generate = function(coverage, callback) {
    ...
    for(var name in coverage) {
      ...
      // processedData[name] = processFile(name, coverage[name]);
  ...
  var compressCoverage = function(testCoverage) {
    ...
    //   var filenameCoverage = coverage[filename];
      // compressed[filename] = {};
      // var compressedCoverage = compressed[filename];
      // var compressedCoverage = {};

      // for(var testname in filenameCoverage) {
        // var testCoverage = filenameCoverage[testname];
        // compressedCoverage[testname] = {s: []};
        compressedCoverage = {s: []};
        // var cStructured = compressedCoverage[testname].s;
        ...
        for(var i = 0; i < testCoverage.structured.length; i++) {
          ...
          if(structured.covered == 'yes') {
            ...
            // var cStruct = {
            //     l: structured.line
            //   , c: structured.covered == 'yes' ? "y" : (structured.coverage == 'neutral' ? 'e' : 'n')
            //   , t: {
            //         c: structured.text.consumeBlanks
            //       , s: structured.text.startPos
            //       , e: structured.text.endPos
            //       , l: structured.text.originalLength
            //     }
            // };
            ...
            if(structured.text.offsets.length > 0) {
              ...
              for(var j = 0; j < structured.text.offsets.length; j++) {
                ...
                // offsets.push([
                //     structured.text.offsets[j].pos
                //   , structured.text.offsets[j].len
                // ])
              }
  ...
  var renderExplorerView = function(keys, coverage, callback) {
    ...
    // Coverage
    var finalCoverage = {tests: testNames, coverage: {}, source: {}};
    ...
    // // zlib.deflate(jsonData, function(err, jsonData) {
    ...
    //   fs.writeFileSync(outputFile + ".json", jsonData, 'utf8');  
