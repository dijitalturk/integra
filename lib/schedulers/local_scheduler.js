...
var LocalScheduler = function(options) {
	...
	this.run = function(tests, configurations, callback) {
		...
		configurations.forEach(function(configuration) {
			testRunners.push(new TestRunner(configuration, options));
		});
		...
		tests.forEach(function(test) {
			...
			// Call the filter method on each filter
			for(var i = 0; i < _filters.length; i++) {
				if(_filters[i].filter && _filters[i].filter(test)) {
			...
			// Call the filter method on each plugin
