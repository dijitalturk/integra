...
var Html = function(options) {
	...
	var setup = function() {
		...
		mkdirIgnore(options.outputDirectory + "/html", 0777);
    ...
    console.log("+++++++++++++++++++++++++++++++++++++")
    console.dir(structured)

  	// var lines = [];
  	// // Get all the text lines
  	// structured.map(function(x) {
  	// 	lines.push(x.text);
  	// });
  	...
  	// var text = lines.join("\n<br/>");
  	// Replace all tabs
  	// text = text.replace(/\t/g, options.tabReplacement);
    ...
    var details = detailTemplate(context);
  	...
  	// Write the file out
  	fs.writeFileSync(options.outputDirectory + "/html/" + outputFilename, html);
	}
}
